...
func SaveSendEmail(tId, cId int64, trackId string) (int64, error) {
	query := `INSERT INTO email_sends (template_id, contact_id, tracking_id, sent_at) VALUES (?, ?, ?, ?) RETURNING id`
	row := db.DB.QueryRow(query, tId, cId, trackId, time.Now())
	var id int64
	err := row.Scan(&id)
	return id, err
}
