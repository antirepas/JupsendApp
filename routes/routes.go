...
func RegisterRoutes(server *gin.Engine) {
	...
	mailboxOnboarding.Use(RequireAuth(), RequireSubscription())
	{
		...
		mailboxOnboarding.POST("/mailboxes/:id/smtp-check", MailboxesSMTPCheck)
		mailboxOnboarding.POST("/mailboxes/:id/dns-check", MailboxesDNSCheck)
		mailboxOnboarding.POST("/mailboxes/:id/mail-tester", MailboxesMailTester)
		mailboxOnboarding.POST("/mailboxes/:id/relink-inboxkit", MailboxesRelinkInboxKit)
	...
	authd.Use(RequireAuth(), RequireSubscription(), RequireMailboxSetup())
	{
		...
		authd.POST("/contacts/lists/:id/members/bulk", BulkListMembers)
		authd.POST("/contacts/lists/:id/members/:contact_id/remove", RemoveListMember)
		authd.POST("/contacts/lists/:id/members/:contact_id/delete", DeleteListMemberContact)
